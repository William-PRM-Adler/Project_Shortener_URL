...
def redirect_url(new_short_url: str, db: Session = Depends(get_database)):
    url_entry = db.query(URLMapping).filter(URLMapping.the_short_url == new_short_url).first()
    if not url_entry:
        raise HTTPException(status_code = 404, detail = "Short URL not found!")
    url_entry.the_click_count += 1
    url_entry.the_last_click = datetime.datetime.utcnow()
    db.commit()
    print(f"Redirecting from {url_entry.the_short_url} to {url_entry.the_long_url} (Click Count: {url_entry.the_click_count})")
